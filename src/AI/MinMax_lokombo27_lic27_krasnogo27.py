...
def utility(gameState):
    ...
    utility = foodScore * 1.05 + attackScore + defenseScore
            ...
            for i, w in enumerate(myWorkers):
                ...
                if w.carrying:
                    ...
                    if dropSites and any(w.coords == d for d in dropSites):
                        ...
                        contrib = 1.0 + 0.5 * progressToDrop
                else:
                ...
                # if our worker is not on our side of the board, punish
                if w.coords[1] > 3:
                    workerScore -= 1111111111
...
def attackUtility(gameState, myInv, enemyInv, me):
    ...
    # Get distance from top left corner of board
    topLeftCorner = (9, 9)
    topLeftCornerScore = 1.0 - (approxDist(myAnts[0].coords, topLeftCorner) 
                                 / maxDist)
  
    return 2.0 + topLeftCornerScore  # no more workers, great
